...
"$app" --post-space
for _ in {1..30}; do
  [[ -e "$marker" ]] && ! fn_is_down && break
  sleep 0.05
done
space_result=""
[[ ! -e "$marker" ]] || space_result=$(<"$marker")
[[ "$space_result" == "plain-space" ]] \
  || { print -u2 -- "KEYBOARD SAFETY RED: receiver did not get a plain Space"; exit 1; }
...
print -- "KEYBOARD SAFETY GREEN: terminal received plain Space and crash watchdog released Fn"
