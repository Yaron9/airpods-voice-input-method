...
if [[ "$launchd_pid" == <-> ]] && kill -0 "$launchd_pid" 2>/dev/null; then
  launchd_executable=$(launchctl print "gui/$(id -u)/$launch_label" 2>/dev/null \
    | awk -F' = ' '/^[[:space:]]*program = / { print $2; exit }' || true)
  ...
  if [[ -z "$launchd_executable" ]]; then
    echo "Could not verify the old background app path; it was left running safely" >&2
    exit 1
  fi
...
if [[ "$installed_pid" == <-> ]] && kill -0 "$installed_pid" 2>/dev/null; then
  print -r -- "$installed_pid" >"$pid_file"
  print -n >"$show_request"
  echo "AirPods Voice 输入法 already running (PID $installed_pid)"
  exit 0
fi
