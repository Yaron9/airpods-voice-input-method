...
为防止全局修饰键影响 Antigravity、Terminal、Todoist 等快捷键敏感应用，1.0 增加了三层保护：启动时清理历史残留 Fn；语音期间一旦检测到实体键盘输入，先移除该事件中的语音修饰键并立即结束语音；App 异常退出时，由独立守护进程释放 Fn。正常停止、切换前台应用、5 分钟安全超时或退出 App 时只释放按键，不会误发送。

如果 macOS 临时拒绝建立键盘监听，默认 `Fn` 配置会继续提供 AirPods 语音功能，并保留启动清理、Fn 状态确认、超时释放和崩溃守护；日志会明确记录键盘保险丝处于降级状态。使用 Control、Option、Command 或 Shift 时，为避免修饰键影响其他 App，键盘监听不可用会拒绝启动。
